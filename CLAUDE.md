...
## 🔒 ЗАМОРОЖЕНІ ФАЙЛИ — НЕ РЕДАГУВАТИ БЕЗ ЯВНОГО ДОЗВОЛУ КОРИСТУВАЧА

Наступні файли містять верифіковану розрахункову логіку. Вони заблоковані від будь-яких змін без явного письмового дозволу Артура в поточній розмові.

| Файл | Що захищено | Верифіковано |
|------|------------|--------------|
| `engine/calculator.py` | Розрахунок позицій планет, транзитів, місячних фаз, VOC | astro.com + pyswisseph |
| `engine/interpreter.py` | ACTIVITY_RULES, пороги scores, get_day_label, is_polarized_day | Ручна верифікація 2026-05-16 |
| `agents/chart_parser.py` | Placidus house assignment (виправлено з Whole Sign), аспекти | astro.com 2026-05-16 |
| `agents/forecast_engine.py` | Транзити, Solar Return з Placidus cusps | Верифіковано 2026-05-16 |

**Правила:**
- ЗАБОРОНЕНО редагувати ці файли без фрази "дозволяю змінити [назва файлу]" від Артура
- Якщо знайдено потенційний баг — ПОВІДОМИТИ, але НЕ виправляти без дозволу
- Якщо зміна потрібна для нової функції — запитати спочатку
- Додавати нові файли можна вільно, але не змінювати логіку в цих чотирьох
